...
static void MODRM_W(cpu_state *cpu, sreg_t *seg, int w, operand_set *set) {
    modrm_t modrm;
    if (MODRM(cpu, seg, &modrm)) {
        if (w) {
...
int cpu_step(cpu_state *cpu) {
    ...
    for (;;) {
        ...
        switch (inst) {
            ...
            {
                modrm_t modrm;
                if (MODRM(cpu, NULL, &modrm)) return 0;
            ...
            case 0x98:  // CBW
                cpu->EAX = MOVSXB(cpu->EAX);
                return 1;

            case 0x99: // CWD
            {
                uint32_t temp = MOVSXW(cpu->EAX);
                cpu->EDX = (temp >> 16);
                return 1;
            }
            ...
            case 0x9E: // SAHF
            {
                uint32_t value = cpu->eflags & 0xFFFFFF00;
                value |= cpu->AH;
                LOAD_FLAGS(cpu, value);
                return 1;
            }
...
void dump_regs(cpu_state *cpu) {
    ...
    *p++ = ':';
    ...
    for (int i = 0; i < 3; i++) {
        *p++ = ' ';
        p = dump8(p, READ_MEM8(&cpu->CS, cpu->EIP + i));
    }
...
void cpu_reset(cpu_state *cpu, int gen) {
    ...
    LOAD_FLAGS(cpu, 0);
    cpu->CS.sel = 0xFFFF;
    cpu->CS.base = 0x000FFFF0;
    cpu->EIP = 0x00000000;
    cpu->CS.limit = 0x0000FFFF;
    cpu->SS.limit = 0x0000FFFF;
    cpu->DS.limit = 0x0000FFFF;
    cpu->ES.limit = 0x0000FFFF;
    cpu->FS.limit = 0x0000FFFF;
    cpu->GS.limit = 0x0000FFFF;
